...
//            amount++;
            ...
            while ((j > 1) && ((toward[j] && (block[j] == j)) || (!toward[j] && (block[j] == 1)))) {// &&-and
                toward[j] = !toward[j];
                j--;
//                елемент рухається вперед і досягає свого крайнього правого положення (очевидно, j не може
//                бути елементом блоку з найменшим елементом, більшим за j).
//                елемент j рухається і досягає свого крайнього лівого становища (у першому блоці).
            }
        }
